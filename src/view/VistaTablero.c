...
void comTableroV(Tablero_v* tablero){
    ...
    for (int i = 0; i < CONTADOR; i++) {
        ...
        // Avanza a la derecha usando el Aanxho real de cada número
    ...
    for (int i = 0; i < TAMAÑO_TABLERO; ++i) {
        ...
        for (int j = 0; j < TAMAÑO_TABLERO; ++j) {
            ...
            if (i == 0 && j == 0) {
                cellW = (float) tablero->celdas[0][0].boat.width;
                cellH = (float) tablero->celdas[0][0].boat.height;
            }

            // Avanza en X usando el ancho de celda conocido
            p.x += (cellW > 0.0f) ? cellW : (float) tablero->celdas[i][j].boat.width;
        }
        // Avanza en Y usando el alto de celda conocido
        p.y += (cellH > 0.0f) ? cellH : (float) tablero->celdas[i][0].boat.height;
    }
}
